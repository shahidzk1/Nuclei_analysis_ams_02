...
def process_tree(file_path, label_frag, label_frag_L0, label_non_frag, charge):
    with TreeHandler(file_path, tree_name, n_jobs) as tree_handler:
        df_frag, df_non_frag = tree_handler.labeled(label_frag, label_non_frag, layer_info, variable_name, rigidity_low, rigidity_up, charge_sel=True, nuclei_charge=charge)
        df_frag_L0, df_non_frag1 = tree_handler.labeled(label_frag_L0, label_non_frag, 0, variable_name, rigidity_low, rigidity_up, charge_sel=True, nuclei_charge=charge)
        del df_non_frag1
    gc.collect()
